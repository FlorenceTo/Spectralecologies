...
class E extends z {
  ...
  _getThumbnailStyles(e) {
    const o = y(e);
    return o ? { backgroundImage: `url(${o})` } : { backgroundImage: "" };
  }
  render() {
    const e = this.viewModel, o = e.state === "disabled" ? null : e.activeBasemap, i = e.state === "disabled" ? null : e.nextBasemap, u = e.state === "loading", s = e.state === "incompatible-next-basemap", l = i?.title ?? "", c = i && i.loadStatus !== "loaded";
    let d;
    const g = this.showTitle && l, p = s, v = g || p, b = this._getThumbnailStyles(o), w = this._getThumbnailStyles(i);
    return v && (d = t`<div class="overlay">${g ? t`<span class=${r(a("title", "title-wrap"))} title=${l ?? n}>${l}</span>` : null}${p ? t`<calcite-scrim class="overlay-scrim" title=${this.messages.incompatibleSpatialReference ?? n}><calcite-icon icon=exclamation-mark-triangle></calcite-icon></calcite-scrim>` : null}</div>`), t`<div class=${r(a("root", m.widget))}><calcite-button appearance=transparent data-basemap-id=${(i ? i.id : "") ?? n} .disabled=${s} kind=neutral .label=${this.label ?? void 0} @click=${() => void this.toggle()} title=${this.label ?? void 0 ?? n}><div class=${r(a("container", "image-secondary"))}><div class="image" style=${h(b)}></div></div><div class="container"><div class=${r(a("image", c ? "image-loading" : null))} style=${h(w)}>${c || u ? t`<calcite-scrim><span aria-hidden=true class=${r(m.loaderAnimation)} role=presentation></span></calcite-scrim>` : null}</div>${d}</div></calcite-button></div>`;
  }
}
