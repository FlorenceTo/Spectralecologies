...
import { LitElement as b, createEvent as x, nothing as y, safeClassMap as M } from "@arcgis/lumina";
...
import S from "@arcgis/core/analysis/AreaMeasurementAnalysis.js";
...
let r = class extends U {
    ...
    this.addHandles([
      ...
      u(
        ...
        ({ analysisView: t, snappingOptions: e }) => {
          t && "snappingOptions" in t && (t.snappingOptions = e);
        },
        { sync: !0, initial: !0 }
      )
    ]);
  ...
  get state() {
    const { analysis: t, disabled: e, operationState: l, ready: s, supported: i } = this, { valid: a } = t;
    return A({ supported: i, disabled: e, ready: s, operationState: l, valid: a });
  }
  get displayState() {
  ...
  get measurement() {
    return {
      ...
      area: m(this.analysis.valid ? this.analysisView?.result?.area : void 0),
      perimeter: m(this.analysis.valid ? this.analysisView?.result?.perimeter : void 0)
    };
  }
  get unitOptions() {
    return E(this._userUnitOptions, h, h);
  }
  set unitOptions(t) {
...
class T extends b {
  ...
  render() {
    const { messages: e, unit: l, unitOptions: s, viewModel: i } = this, { measurement: a, state: d } = i;
    return c`<div aria-label=${e.componentLabel ?? y} class=${M({
      [p.widget]: !0,
      [p.panel]: !0
    ...
    ]}>${g({ hidden: this.hideUnitSelect, children: c`<arcgis-unit-select slot=settings .options=${s} .selectLabel=${e.unit} .value=${l} @arcgisUnitSelectChange=${(w) => {
      this.unit = w.detail;
...
f("arcgis-area-measurement-2d", T);
export {
  T as ArcgisAreaMeasurement2d
};
