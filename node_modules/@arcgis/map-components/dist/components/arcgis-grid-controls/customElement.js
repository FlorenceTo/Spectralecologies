...
import { css as $, html as r } from "lit";
...
class W extends E {
  ...
  render() {
    return r`<div class=${p(g("root", v.widget, v.panel))}><div class="container">${this._renderContent()}</div></div>`;
  }
  _renderContent() {
    const { hideGridEnabledToggle: e, hidePlacementButtons: i, hideNumericInputs: t, hideLineIntervalInput: a, hideGridSnapEnabledToggle: o, hideDynamicScaleToggle: l, hideRotateWithMapToggle: s, hideColorSelection: c } = this;
    return [
  ...
  _renderGridPlacementButtons() {
    const { interactivePlacementState: e, placementDisabled: i, gridControlsEnabled: t } = this, a = i || !t;
    return r`<calcite-label scale=s>${this._messages.placementOptions}<calcite-action-bar expand-disabled layout=horizontal>${B.map((o) => this._renderPlacementModeButton(o, a, e))}</calcite-action-bar></calcite-label>`;
  }
  _renderPlacementModeButton(e, i, t) {
    const a = this._messages?.placementState?.[e], o = this._placementModeButtonRefs.get(e);
    return r`<div><calcite-action .active=${t === e} alignment=center .disabled=${i} .icon=${h.placementState[e]} data-placement-action=${e ?? m} @click=${() => this._togglePlacementState(e)} .text=${a ?? ""} ${f(this._setPlacementButtonRef)}></calcite-action>${o ? r`<calcite-tooltip overlay-positioning=fixed placement=top .referenceElement=${o}>${a}</calcite-tooltip>` : void 0}</div>`;
  }
  _renderNumericInputs() {
    const { rotation: e, spacing: i, effectiveSpacingAfterDynamicScaling: t, numericSpacingInputShouldBeVisible: a, gridControlsEnabled: o } = this, l = t != null && t !== i;
    return r`<div class=${p(g("numeric-inputs-container"))}>${[
      a ? r`<calcite-label scale=s><span>${this._messages?.inputLabel?.spacing}${l ? r`<calcite-icon class=${p(g("inline-icon"))} .icon=${h.warning} scale=s tabindex=0 ${f(this._setWarningIconRef)}></calcite-icon>` : void 0}</span><calcite-input-number .disabled=${!o} number-button-type=none .value=${i?.toFixed(2)} @calciteInputNumberChange=${({ currentTarget: s }) => {
        const c = Number.parseFloat(s.value);
        this.spacing = c, c !== this.spacing && (s.value = `${this.spacing}`);
      }}></calcite-input-number>${l && this._warningIconTooltipRef ? r`<calcite-tooltip overlay-positioning=fixed placement=leading .referenceElement=${this._warningIconTooltipRef}>${P(this._messages?.warnings?.dynamicSpacing ?? "", { actualSpacingIncludingUnit: t }, {
        format: {
      ...
      r`<calcite-label scale=s>${this._messages?.inputLabel?.angle}<calcite-input-number .disabled=${!o} integer number-button-type=none suffix-text=\u00B0 .value=${`${L(e)}`} @calciteInputNumberChange=${({ currentTarget: s }) => this.rotation = D(s.value)}></calcite-input-number></calcite-label>`
    ]}</div>`;
  }
  _renderIntervalInput() {
    const { majorLineInterval: e, gridControlsEnabled: i } = this;
    return r`<div class=${p(g("numeric-inputs-container"))}><calcite-label scale=s>${this._messages?.inputLabel?.interval}<calcite-input-number .disabled=${!i} integer max=15 min=1 number-button-type=vertical .value=${`${e}`} @calciteInputNumberChange=${({ currentTarget: t }) => {
      const a = Number.parseInt(t.value, 10);
      this.majorLineInterval = a, a !== this.majorLineInterval && (t.value = `${this.majorLineInterval}`);
  ...
  _renderSnappingToggle() {
    ...
    return r`<arcgis-labeled-switch .checked=${!!t} .disabled=${!(i && !!e)} .label=${this._messages?.switchLabel?.snap} @arcgisCheckedChanged=${(o) => {
      o.stopPropagation(), this.snappingOptions?.set("gridEnabled", o.detail);
    }}></arcgis-labeled-switch>`;
  }
  _renderDynamicScaleToggle() {
    const { _messages: e, gridOutOfScale: i, dynamicScaling: t, gridControlsEnabled: a } = this, o = !this.hideOutOfScaleWarning && i;
    return r`<arcgis-labeled-switch .checked=${t} .disabled=${!a} .hint=${o ? e?.warnings?.outOfScale : void 0} .hintIcon=${h.warning} hint-kind=warning .label=${e?.switchLabel?.scaling} @arcgisCheckedChanged=${(l) => {
      l.stopPropagation(), this.dynamicScaling = l.detail;
    }}></arcgis-labeled-switch>`;
  }
  _renderRotateToggle() {
    const { gridControlsEnabled: e, rotateWithMap: i } = this;
    return r`<arcgis-labeled-switch .checked=${i} .disabled=${!e} .label=${this._messages?.switchLabel?.rotateWithMap ?? ""} @arcgisCheckedChanged=${(t) => {
      t.stopPropagation(), this.rotateWithMap = t.detail;
    }}></arcgis-labeled-switch>`;
  }
  _renderEnabledToggle() {
    return r`<div class="switch--bordered"><arcgis-labeled-switch .checked=${this.displayEnabled} .disabled=${!!this.interactivePlacementState} .label=${this._messages?.switchLabel?.display} @arcgisCheckedChanged=${(e) => {
  ...
  _renderColorSelection() {
    ...
    return r`<calcite-label scale=s>${this._messages?.inputLabel?.theme}<div class="tile-group">${[
      this._renderThemeTile({ value: "dark" }),
      this._renderThemeTile({ value: "light" }),
      this._renderThemeTile({ value: "custom" }),
      this._themePopoverRef ? r`<calcite-popover auto-close .label=${this._messages?.gridLineColorPopoverLabel ?? ""} overlay-positioning=fixed .referenceElement=${this._themePopoverRef}><calcite-color-picker alpha-channel saved-disabled .value=${e?.toHex({ digits: 8 }) ?? "#000000ff"} @calciteColorPickerChange=${(i) => {
  ...
  _renderThemeTile(e) {
    const { theme: i, gridControlsEnabled: t } = this, { value: a } = e, o = this._messages?.gridTheme?.[a], l = `${this._labelUid}--theme_tile_${a}`, s = `${l}__label`, c = h.gridTheme[a];
    return r`<div class=${p(g({ tile: !0, disabled: !t }))} role=button ${f(a === "custom" ? this._setPopoverRef : this._emptyFunction)}><input aria-labelledby=${s ?? m} .checked=${i === a} .disabled=${!t} id=${l ?? m} .name=${`${this._labelUid}--theme`} @change=${(x) => {
      this.theme = a, x.currentTarget.focus();
