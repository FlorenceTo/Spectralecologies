...
class B extends v {
  ...
  set selectedSubnetworkName(e) {
    this._unTraceConfiguration && (this._unTraceConfiguration.subnetworkName = e);
  }
  get _allLayers() {
    const { view: e } = this;
    return e?.map ? e.map.allLayers.toArray().concat(e.map.allTables.toArray()) : [];
  }
  get showStartingPoints() {
    return !this.isSubnetworkNameTraceSelected;
  }
  get showStoppingPoints() {
    const e = this.selectedNamedTraceConfiguration?.traceType;
  ...
  async destroy() {
    this._removeGraphicLayer(p), this._removeGraphicLayer(n), this._sketchViewModel?.cancel(), this._featuresComponentElement?.destroy(), this._traceLocationStartingElement?.destroy(), this._traceLocationBarrierElement?.destroy(), this._traceLocationStoppingElement?.destroy(), await this.manager.destroy();
  }
  load() {
  ...
  _clearTraceLocations() {
    this._getGraphicLayer(n)?.removeAll(), this._startingPoints.removeAll(), this._barriers.removeAll(), this._stoppingPoints.removeAll();
  }
  ...
  _setupSketchViewModel() {
    this._createGraphicLayer(p), this._createGraphicLayer(n), this._sketchViewModel = this._initializeSketchViewModel(), this._setSnappingOption();
  }
  _clearSubnetworks() {
  ...
  async _loadSubnetworksTable() {
    this.subnetworksTable || (this.subnetworksTable = await this.selectedUtilityNetwork?.loadSubnetworksTable());
  }
  async _querySubnetworks() {
    const { subnetworksTable: e } = this;
    if (!e)
      return;
  ...
  async _handleSubnetworkTraceConfigChange(e) {
  ...
  _selectTraceConfigurationByGuid(e) {
  ...
  _handleTraceConfigChange() {
    ...
    switch (this._unTraceConfiguration = e.traceConfiguration.clone(), e.traceType) {
      case "subnetwork":
        this._state = "loading", this._handleSubnetworkTraceConfigChange(this._unTraceConfiguration).finally(() => this._state = "ready");
        break;
    }
  }
  ...
  _filterUnLayers() {
    ...
    this._unLayers = new l(this._allLayers.filter((t) => e?.isUtilityLayer(t) && !this._subnetworkLayerIds.includes(t.layerId))), this._setSnappingOption();
  }
  ...
  async _setupUtilityNetwork() {
    if (this.view?.map) {
      ...
      await e.load(), this.utilityNetworks = e.utilityNetworks || void 0;
    } else
      this._state = "disabled", u("error", this, "The property `view.map` is required to load utility networks but is missing.");
  }
  ...
  _getLayerByLayerId(e) {
    if (e == null)
      return;
    const { _layerIdToLayer: t, _unLayers: i } = this;
    if (t.get(e))
      return t.get(e);
    if (!i)
      return;
    const a = i.find((r) => "layerId" in r && r.layerId === e);
  ...
  async _queryFeatures(e, t) {
    ...
    return a.forEach((r) => {
      ...
      const o = r.layer;
      r.getEffectivePopupTemplate(this._defaultPopupTemplateEnabled) || (r.popupTemplate = o.createPopupTemplate());
    }), a;
  }
  async _populateFeaturesWithLabel(e) {
    const t = /* @__PURE__ */ new Map();
    e.forEach((a) => {
      const r = a.layer?.type === "subtype-sublayer" ? a.layer.parent : a.layer;
      if (r) {
        const o = r.layerId, c = this._getLayerByLayerId(o);
        if (c) {
          const g = t.get(c) || [];
          g.push(a), t.set(c, g);
        }
    ...
    const i = [];
    return await Promise.all(Array.from(t.keys()).map(async (a) => {
      const r = t.get(a) ?? [], o = await this._queryFeatures(a, r);
      i.push(...o);
  ...
  async _handleHitTestComplete(e) {
    if (this._exceedDisplayLimit = e.length > this._displayLimit, this._featuresFromHitTest = await this._populateFeaturesWithLabel(e.slice(0, this._displayLimit)), this._featuresFromHitTest.length === 0)
  ...
  renderUtilityNetworkSelection() {
    const { _messages: e } = this, t = e.labels.utilityNetwork, i = this.utilityNetworks?.map((a) => s`<calcite-option .value=${a.id}>${a.title}</calcite-option>`);
    return s`<calcite-block .heading=${t} .label=${t} expanded><calcite-select .label=${t} .value=${this.selectedUtilityNetwork?.id} @calciteSelectChange=${(a) => {
      this.selectedUtilityNetwork?.id !== a.target.value && (this.selectedUtilityNetwork = this.utilityNetworks?.find((r) => r.id === a.target.value));
    }}>${i}</calcite-select></calcite-block>`;
  }
  renderDefaultMode() {
    const { _messages: e, _state: t, mode: i } = this;
    return s`<calcite-flow-item .selected=${i === "default"}>${t === "disabled" ? this.renderError(e.errors.noMapError) : this.renderContent()}</calcite-flow-item>`;
  }
  renderSpatialSelectInstructions() {
    const { _messages: e, _activeTraceLocationType: t, mode: i } = this, a = t === "starting-point" ? e.labels.addSpatial.startingPoint : t === "barrier" ? e.labels.addSpatial.barrier : e.labels.addSpatial.stoppingPoint;
    return s`<calcite-flow-item .selected=${i === "add-spatial-instructions"} .heading=${a} @calciteFlowItemBack=${(r) => this._handleAddSpatialCancelClick(r)}><div class="add-spatial-mode"><div><h5 class="heading">${e.instructions.addSpatial.selectFeature.heading}</h5>${e.instructions.addSpatial.selectFeature.subheading}</div><div><h5 class="heading">${e.instructions.addSpatial.selectAssociatedFeature.heading}</h5>${e.instructions.addSpatial.selectAssociatedFeature.subheading}</div></div></calcite-flow-item>`;
  }
  renderFeaturesList(e) {
    return s`<calcite-list label="Feature list">${e.map(this.renderFeatureListItem)}</calcite-list>`;
  }
  renderHitTestResults() {
    const { _messages: e, _activeTraceLocationType: t, _featuresFromHitTest: i, mode: a } = this, r = t === "starting-point" ? e.labels.addSpatial.startingPoint : t === "barrier" ? e.labels.addSpatial.barrier : e.labels.addSpatial.stoppingPoint;
    return s`<calcite-flow-item .selected=${a === "add-spatial-feature-list"} .heading=${r} @calciteFlowItemBack=${(o) => this._handleAddSpatialCancelClick(o)}>${this.renderFeaturesList(i)}</calcite-flow-item>`;
  }
  ...
  renderAddSpatialFooter() {
    ...
    }), o = e === "add-spatial-feature-list" && this._exceedDisplayLimit, c = a ? !(i?.isUtilityLayer(a.layer) ?? !0) : !0;
  ...
  renderSubnetworkSelectList() {
    return s`<calcite-select .label=${this._messages.labels.subnetworkName} .value=${this.selectedSubnetworkName} @calciteSelectChange=${(e) => this.selectedSubnetworkName = e.target.value}>${this.compatibleSubnetworkNames.map((e) => s`<calcite-option .value=${e}>${e}</calcite-option>`)}</calcite-select>`;
  }
  ...
  renderSubnetworkInfo() {
    ...
    return i && t?.traceType && this._subnetworkTraceTypes.has(t.traceType) ? s`<calcite-block id=subnetwork-info-block expanded .label=${e.labels.subnetworkInfo}><arcgis-utility-network-subnetwork-info .namedTraceConfig=${t}></arcgis-utility-network-subnetwork-info></calcite-block>` : null;
  }
  renderContent() {
    const { utilityNetworks: e } = this, t = this.selectedNamedTraceConfiguration?.traceConfiguration;
    if (!e || e.length === 0)
      return this.renderError(this._messages.errors.noUtilityNetworkError);
    const i = s`${this.renderTraceSelection()}${this.renderSubnetworkInfo()}${this.renderSubnetworkSelection()}`, a = s`${this.showStartingPoints ? this.renderStartingPointSelection() : null}${this.showStoppingPoints ? this.renderStoppingPointSelection() : null}${this.renderBarrierSelection()}`;
    return s`<calcite-block-group .label=${this._messages.labels.traceInputs}>${e.length > 1 ? this.renderUtilityNetworkSelection() : null}${this.selectedUtilityNetwork?.loaded ? i : this.renderLoader()}${t ? a : null}</calcite-block-group>`;
  }
  render() {
    const { mode: e } = this;
