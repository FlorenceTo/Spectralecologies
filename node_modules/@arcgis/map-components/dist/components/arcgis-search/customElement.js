...
import v from "@arcgis/core/widgets/Search/SearchViewModel.js";
import { LitElement as w, createEvent as f, renderElement as b, safeClassMap as x, nothing as M } from "@arcgis/lumina";
...
class L extends w {
  ...
  async search(e) {
    ...
    try {
      const r = await this.viewModel.search(e, { signal: i });
      return this._searchController !== t ? void 0 : (r?.numResults || (this._showNoResults = !0, this.activeMenu = "suggestion"), this._searchController = void 0, r);
    } catch {
  ...
  async suggest(e) {
    ...
    try {
      const r = await this.viewModel.suggest(e, null, { signal: i });
      return this._suggestController !== t ? void 0 : (this._suggestController = void 0, r?.numResults && (this.activeMenu = "suggestion", this._scrollToTopSuggestion()), r);
    } catch {
    ...
    this.manager.onLifecycle(() => [
      d(() => [this._searchResultRendererEl, this.viewModel?.defaultPopupTemplate], () => {
        ...
        if (e && t) {
          ...
          i.appendChild(t), e.content = i, this._searchResultRendererContentEl = t;
        }
      }, { initial: !0 }),
  ...
  _openPopup() {
    if (h(this.referenceElement)) {
      ...
      const { resultGraphic: e, resultLocation: t } = this.viewModel;
      this.view.openPopup({
        features: e ? [e] : [],
        location: t
      });
  ...
  _handleCalciteAutocompleteTextInput(e) {
    const i = e.target.inputValue;
    i !== this.viewModel.searchTerm && (this.viewModel.searchTerm = i, i || this.clear(), this.suggest());
  }
  ...
  _handleCalciteAutocompleteChange(e) {
    ...
    const i = JSON.parse(t), r = this.viewModel.suggestions?.find(({ sourceIndex: s }) => s === i.sourceIndex)?.results?.find(({ key: s }) => `${s}` == `${i.key}`);
    r && this.search(r);
  }
  async _useCurrentLocation() {
    ...
    try {
      ...
      C(i) || (this._locateFailed = !0, this.activeMenu = "suggestion");
    } finally {
      this._searchController = void 0;
    }
  }
  ...
  _handleSourceSelect(e) {
    const r = e.target.selectedItems[0].getAttribute("data-source-index");
    r && (this.viewModel.activeSourceIndex = Number.parseInt(r, 10));
  }
  _cancelSuggest() {
    this.activeMenu = "none", this._locateFailed = !1, this._showNoResults = !1, this._suggestController = u(this._suggestController);
  }
  _cancelQueries() {
    this._cancelSuggest(), this._searchController = u(this._searchController);
  }
  ...
  _renderAutocomplete() {
    const { _effectiveActiveMenu: e, messages: t, _isDisabled: i } = this, { maxInputLength: r, placeholder: s, searchTerm: n, state: c } = this.viewModel, g = this.label ?? t.searchButtonTitle ?? "";
    return o`<calcite-autocomplete autocomplete=off class="autocomplete" .disabled=${i} .icon=${this.icon || void 0} .inputValue=${n} .label=${g} .loading=${c === "searching"} .maxLength=${r} .name=${this.el.id} @keydown=${this._handleInputKeydown} .open=${e === "suggestion"} overlay-positioning=fixed .placeholder=${s} .topLayerDisabled=${this.topLayerDisabled} title=${(n ? "" : s) ?? M} @calciteAutocompleteChange=${this._handleCalciteAutocompleteChange} @calciteAutocompleteClose=${this._handleAutocompleteClose} @calciteAutocompleteOpen=${this._handleAutocompleteOpen} @calciteAutocompleteTextChange=${this._handleCalciteAutocompleteTextInput} @calciteAutocompleteTextInput=${this._handleCalciteAutocompleteTextInput} ${_(this._handleAutocompleteCreate)}>${this._renderSuggestions()}${this._renderNotices()}</calcite-autocomplete>`;
  }
  _renderForm() {
  ...
  _renderSuggestResults(e) {
    const { allSources: t, activeSourceIndex: i } = this.viewModel, { sourceIndex: r } = e, s = e.results?.length, n = t.length > 1 && i === -1, c = this._getSourceName(r);
    return s && n ? o`<calcite-autocomplete-item-group .heading=${c}>${this._renderSuggestGroup(e)}</calcite-autocomplete-item-group>` : this._renderSuggestGroup(e);
  }
  _renderSuggestion({ key: e, sourceIndex: t, text: i }) {
    const r = {
      key: e,
      sourceIndex: t
    };
    return o`<calcite-autocomplete-item .heading=${i ?? this.messages.untitledResult ?? ""} .value=${JSON.stringify(r)}></calcite-autocomplete-item>`;
  }
  _renderSource(e) {
  ...
  _renderNoResultsWarning(e) {
    const { messages: t } = this, i = e ? R(t.noResultsFoundForValue ?? "", {
      value: `"${e}"`
    }) : t.noResultsFound;
