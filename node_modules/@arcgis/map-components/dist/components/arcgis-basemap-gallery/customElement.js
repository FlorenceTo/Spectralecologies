...
class R extends b {
  constructor() {
    super(...arguments), this.messages = k({}), this.viewModel = B(this), this._width = 0, this.view = this.viewModel.view, this.activeBasemap = this.viewModel.activeBasemap, this.autoDestroyDisabled = !1, this.disabled = !1, this.headingLevel = 2, this.icon = "basemap", this.label = "", this.source = this.viewModel.source, this.state = this.viewModel.state, this.arcgisPropertyChange = y()("activeBasemap", "state"), this.arcgisReady = _();
  }
  static {
    this.properties = { _width: 16, view: 0, activeBasemap: 1, autoDestroyDisabled: 5, disabled: 7, headingLevel: 9, icon: 1, label: 1, messageOverrides: 0, referenceElement: 1, source: 0, state: 32 };
  }
  static {
    this.styles = $;
  }
  ...
  _handleKeyDown(i) {
    ...
    if (!["ArrowUp", "ArrowDown", "ArrowRight", "ArrowLeft"].includes(e))
      return;
    i.preventDefault();
    const { items: t, activeBasemapIndex: r } = this.viewModel, c = e === "ArrowUp" || e === "ArrowLeft" ? this._getRoundRobinIndex(Math.max(r - 1, -1), t.length) : this._getRoundRobinIndex(r + 1, t.length), s = t.at(c);
    s?.state === "ready" && (this.viewModel.activeBasemap = s.basemap);
  }
  render() {
    ...
    t["layout-grid"] = r >= l.wide || r <= l.small, t["narrow-items"] = r < l.default;
    const c = i ? a`<div class="loader"></div>` : null, s = i ? null : o.length > 0 ? a`<div aria-disabled=${this.disabled ?? d} aria-label=${this.label ?? d} class="item-container" @keydown=${this._handleKeyDown} role=radiogroup>${o.map((g, m) => a`<arcgis-basemap-gallery-item .disabled=${e} .item=${g} .itemIndex=${m} .viewModel=${this.viewModel} .messages=${this.messages}></arcgis-basemap-gallery-item>`)}</div>` : a`<div class=${p(n.empty)}>${M({ level: this.headingLevel, children: this.messages.noBasemaps })}</div>`;
    return a`<div class=${p(u("root", n.widget, n.panel, t))} style=${x({
      "--esri-basemap-gallery-small": `${l.small}px`
    })}>${[c, s]}</div>`;
  }
}
h("arcgis-basemap-gallery", R);
export {
