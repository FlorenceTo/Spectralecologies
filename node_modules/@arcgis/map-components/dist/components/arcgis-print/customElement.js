...
import { LitElement as m, createEvent as a, safeClassMap as w } from "@arcgis/lumina";
...
class x extends m {
  ...
  addPortalTemplate(e) {
    return this.viewModel.addPortalTemplate(e);
  }
  async destroy() {
  ...
  loaded() {
    this.manager.onLifecycle(() => [
      s(() => this.viewModel.error, (i) => this.error = i),
      s(() => this.viewModel.state, (i) => {
        i === "ready" && this.arcgisReady.emit();
      }),
      this.viewModel.browseTemplates.on("change", (i) => {
        this.arcgisBrowseTemplatesChange.emit(i);
      }),
      s(() => [this.printServiceUrl, this.portal], () => {
        this.viewModel.load();
      }, { initial: !0 })
    ]);
  ...
  render() {
    const e = {
      "panel-items-centered": this.viewModel.state === "initializing"
    };
    return r`<div class=${w(g("root", n.widget, n.panel, e))}>${this._renderPanel()}</div>`;
  }
  _renderPanel() {
    return this.viewModel.state === "disabled" || this.viewModel.state === "initializing" ? r`<calcite-loader .label=${this.messages.loading} scale=m></calcite-loader>` : r`<calcite-flow><calcite-flow-item .selected=${!this._showTemplates}><div class="container">${this._renderPanelHeader()}${this._renderPanelContent()}</div></calcite-flow-item><calcite-flow-item .heading=${this.messages.chooseTemplate} .selected=${this._showTemplates} @calciteFlowItemBack=${(e) => {
      e.preventDefault(), this._showTemplates = !1;
    }}><arcgis-print-template-selector .messages=${this.messages} .viewModel=${this.viewModel} .hideTemplateSelector=${this._hideTemplateSelector.bind(this)} .excludeOrganizationTemplates=${this.excludeOrganizationTemplates}><slot name=browse-template-button></slot></arcgis-print-template-selector></calcite-flow-item></calcite-flow>`;
  }
  _renderPanelHeader() {
    return this.hideHeader ? null : r`<header class="header-title">${this.messages.export}</header>`;
  }
  _renderPanelContent() {
