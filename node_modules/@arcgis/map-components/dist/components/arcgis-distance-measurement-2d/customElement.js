...
import { LitElement as v, createEvent as b, nothing as x, safeClassMap as y } from "@arcgis/lumina";
...
let r = class extends D {
  ...
  initialize() {
    this.addHandles([
      ...
      u(
        ...
        ({ analysisView: t, snappingOptions: e }) => {
          t && (t.snappingOptions = e);
        },
        { sync: !0, initial: !0 }
      )
    ]);
  ...
  get state() {
    const { analysis: t, disabled: e, operationState: a, ready: o, supported: i } = this, { valid: l } = t;
    return U({ supported: i, disabled: e, ready: o, operationState: a, valid: l });
  }
  get displayState() {
  ...
  get measurement() {
    return {
      mode: this.analysisView?.result?.mode ?? "euclidean",
      length: E(this.analysis.valid ? this.analysisView?.result?.length : void 0)
    };
  }
  get unitOptions() {
    return j(this._userUnitOptions, m, m);
  }
  set unitOptions(t) {
    this._userUnitOptions = t;
  }
  get unit() {
    return A(this._userUnit, this.analysis.unit, this.unitOptions, this.defaultMeasurementSystem);
  }
  set unit(t) {
    this._userUnit = t;
  }
  async place(t) {
    this.analysisView && (await this.analysisView.place(t), this.analysisView.interactive = !0);
  }
  clearAnalysis() {
...
r = s([
  $("components.distance-measurement-2d.DistanceMeasurement2dViewModel")
], r);
const T = O(r);
class Z extends v {
  constructor() {
    super(...arguments), this.viewModel = T(this), this.messages = z({ name: "distance-measurement-2d", blocking: !0 }), this.view = this.viewModel.view, this.analysis = this.viewModel.analysis, this.autoDestroyDisabled = !1, this.hideStartButton = !1, this.hideUnitSelect = !1, this.hideVisualization = this.viewModel.hideVisualization, this.icon = "measure-line", this.snappingOptions = this.viewModel.snappingOptions, this.state = this.viewModel.state, this.unit = this.viewModel.unit, this.unitOptions = this.viewModel.unitOptions, this.arcgisPropertyChange = M()("analysis", "state"), this.arcgisReady = b();
  }
  static {
  ...
  async start() {
    V(this.viewModel.start());
  }
  render() {
    const { messages: e, unit: a, unitOptions: o, viewModel: i } = this, { measurement: l, state: c } = i;
    return d`<div aria-label=${e.componentLabel ?? x} class=${y(k("root", p.widget, p.panel))} role=presentation><arcgis-measurement-content .state=${i.displayState} .hintLabel=${e.hint} .unsupportedLabel=${e.unsupported} .items=${[
      {
        title: e.distance,
        measurement: l.length
      }
    ]}>${g({ hidden: this.hideUnitSelect, children: d`<arcgis-unit-select slot=settings .options=${o} .selectLabel=${e.unit} .value=${a} @arcgisUnitSelectChange=${(_) => {
      this.unit = _.detail;
    }}></arcgis-unit-select>` })}${g({ hidden: this.hideStartButton, children: d`<calcite-button slot=actions .disabled=${c === "unsupported" || c === "disabled"} @click=${this.start}>${e.newMeasurement}</calcite-button>` })}</arcgis-measurement-content></div>`;
  }
}
w("arcgis-distance-measurement-2d", Z);
export {
  Z as ArcgisDistanceMeasurement2d
};
