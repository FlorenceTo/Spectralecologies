...
import { css as _, html as c } from "lit";
...
import { formatNumber as y } from "@arcgis/core/intl.js";
import k from "@arcgis/core/widgets/ScaleBar/ScaleBarViewModel.js";
import { LitElement as z, createEvent as C, safeClassMap as l, safeStyleMap as f } from "@arcgis/lumina";
...
class B extends z {
  ...
  loaded() {
    this.manager.onLifecycle(() => x(() => ({
      defaultUnit: this.viewModel.view ? w(this.viewModel.view) : void 0,
  ...
  _handleRootCreateOrUpdate() {
    ...
    if (!this.viewModel && !e.childElem)
      return;
    const i = e.childElem?.getBoundingClientRect();
    if (!i)
      return;
    const { scaleComputedFrom: r } = this.viewModel, n = i.left + window.pageXOffset, t = i.top + window.pageYOffset, o = $(n, t);
  ...
  render() {
    ...
    let r, n;
    if (!e) {
      const { unit: t, barStyle: o } = this, d = t === "imperial" || t === "dual", u = t === "metric" || t === "dual", g = 100, p = t === "dual" ? "line" : o;
      if (d) {
        const a = this.viewModel.getScaleBarProperties(g, "imperial");
        a && (n = p === "ruler" ? this._renderRuler(a) : this._renderLine(a, "bottom"));
      }
      if (u) {
        const a = this.viewModel.getScaleBarProperties(g, "metric");
        a && (r = p === "ruler" ? this._renderRuler(a) : this._renderLine(a, "top"));
      }
    }
    return c`<div class=${l(s("root", b.widget, i))} ${v(this._handleContainerRef)}>${r}${n}</div>`;
  }
  _renderRuler(e) {
    const { length: i, unit: r, value: n } = e, t = Math.round(i), o = this._formatDistance(n, r);
    return c`<div class=${l(s("bar-container", "bar-container-ruler"))}><div class="ruler" style=${f({ width: `${t}px` })}></div><div class=${l(s("label-container", "label-container-ruler"))}><div class="label">0</div><div class="label">${o}</div></div></div>`;
  }
  _renderLine(e, i) {
    const { length: r, unit: n, value: t } = e, o = Math.round(r), d = this._formatDistance(t, n), g = c`<div class=${l(s("label-container", "label-container-line", {
      "label-container-top": i === "top",
      "label-container-bottom": i === "bottom"
    }))}><div class="label">${d}</div></div>`, a = c`<div class=${l(s("line", {
      "line-top": i === "top",
      "line-bottom": i === "bottom"
    }))} style=${f({ width: `${o}px` })}></div>`;
    return c`<div class=${l(s("bar-container", "bar-container-line"))}>${[a, g]}</div>`;
  }
}
m("arcgis-scale-bar", B);
export {
  B as ArcgisScaleBar
};
