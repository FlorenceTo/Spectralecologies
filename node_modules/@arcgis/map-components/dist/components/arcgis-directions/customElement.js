...
import c from "@arcgis/core/widgets/Directions.js";
...
class u extends l {
  ...
  async getDirections() {
    if (this.widget?.viewModel?.state !== "ready")
      throw new Error(`${t} - component not ready to get directions`);
    return await this.widget?.getDirections();
  }
  async save() {
    if (this.widget?.viewModel?.state !== "ready")
      throw new Error(`${t} - component not ready to save directions`);
    return await this.widget?.save();
  }
  async saveAs(i, e) {
    if (this.widget?.viewModel?.state !== "ready")
      throw new Error(`${t} - component not ready to save directions as a Portal Item`);
    return await this.widget?.saveAs(i, e);
  }
  async zoomToRoute() {
    if (this.widget?.viewModel?.state !== "ready")
      throw new Error(`${t} - component not ready to zoom to route`);
    this.widget?.zoomToRoute();
  }
  willUpdate(i) {
    if (i.has("routeLayerItemId")) {
      const e = this.routeLayerItemId;
      if (p(e) && !this.useDefaultRouteLayer) {
        const { layer: r } = this;
        r && this.widget.view?.map?.remove(r), this._setDefaultLayer();
      } else e && this.widget && (this.layer = new o({
        portalItem: {
          id: e
        }
  ...
  _setDefaultLayer() {
    this.useDefaultRouteLayer && !this.routeLayerItemId && (this.layer = new o(), n(() => this.widget.view, () => this.widget.view?.map?.add(this.layer), { once: !0 }));
  }
}
